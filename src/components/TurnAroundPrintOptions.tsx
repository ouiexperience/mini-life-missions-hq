...
const TurnAroundPrintOptions: React.FC = () => {
  const { missions, isMissionCompleted } = useTurnAround();
  ...
  const generatePrintContent = (filterType: 'all' | 'completed' | 'incomplete') => {
    ...
    const htmlContent = `
      ...
      <html>
      <head>
        <title>Turn Around Moments - ${selectedChild.name}</title>
        <style>
          body {
      ...
      <body>
        <h1>Turn Around Moments for ${selectedChild.name}</h1>
        <p style="text-align: center;">Printed on ${new Date().toLocaleDateString()}</p>
        
        ${filteredMissions.map(mission => {
          const categoryClass = mission.category === 'Everyday Battles' 
            ? 'mission-money' 
            : mission.category === 'Big Emotions' 
              ? 'mission-health' 
              : 'mission-communication';
...
export default TurnAroundPrintOptions;
