...
const TurnAroundMissionBrowser: React.FC = () => {
  ...
  return <div className="mb-8">
      <div className="flex flex-col gap-4 mb-6">
        ...
        <div className="flex flex-col gap-2">
          ...
          <div className="flex flex-wrap gap-2">
            <Button variant={selectedCategory === null ? "default" : "outline"} size="sm" onClick={() => handleCategoryFilter(null)} className="bg-orange-500 hover:bg-orange-400">
              All
            </Button>
            <Button variant={selectedCategory === "Everyday Battles" ? "default" : "outline"} size="sm" className={selectedCategory === "Everyday Battles" ? "bg-mission-money text-white" : ""} onClick={() => handleCategoryFilter("Everyday Battles")}>
              Everyday Battles
            </Button>
            <Button variant={selectedCategory === "Big Emotions" ? "default" : "outline"} size="sm" className={selectedCategory === "Big Emotions" ? "bg-mission-health text-white" : ""} onClick={() => handleCategoryFilter("Big Emotions")}>
              Big Emotions
            </Button>
            <Button variant={selectedCategory === "Power Struggles" ? "default" : "outline"} size="sm" className={selectedCategory === "Power Struggles" ? "bg-mission-communication text-white" : ""} onClick={() => handleCategoryFilter("Power Struggles")}>
              Power Struggles
            </Button>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {filteredMissions.length > 0 ? filteredMissions.map(mission => <TurnAroundMissionCard key={mission.id} mission={mission} />) : <div className="col-span-full text-center p-8 border rounded-lg">
